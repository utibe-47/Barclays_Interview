...
from . import auth
from .. import db, primary_users, mail
...
def send_reset_email(user):
    ...
    request'''.format(url_for('auth.reset_token', token=token, _external=True))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and user.check_password(form.password.data):
            ...
            return redirect(request.args.get('next') or url_for('funds.funds_home'))
...
@auth.route("/request_token/<token>", methods=["GET", "POST"])
