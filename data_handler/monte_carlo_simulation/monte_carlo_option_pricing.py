...
class MonteCarloOptionPricing(BaseModel):
    ...
    @property
    def lookback_option(self) -> List[float]:
        """Calculate lookback option payoff with fixed strike price"""
        s = self.simulatepath

        a = s.max(axis=0)
        b = s.min(axis=0)

        lookback_call = self.df * np.mean(np.maximum(0, a - self.K))
        lookback_put = self.df * np.mean(np.maximum(0, self.K - b))

        return [lookback_call, lookback_put]

    @property
    def lookback_floating_option(self) -> List[float]:
        """Calculate lookback option payoff with floating strike price"""
        s = self.simulatepath

        a = s.max(axis=0)
        b = s.min(axis=0)

        lookback_call = self.df * np.mean(np.maximum(0, s[-1] - b))
        lookback_put = self.df * np.mean(np.maximum(0, a - s[-1]))
